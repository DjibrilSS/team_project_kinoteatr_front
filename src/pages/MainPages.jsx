      ...
      <div className={styles.categoryfilters}>
        <div className={styles.categoryfilters_item}>
          {categories.map((item,i)=>{
            return <div key={i} onClick={() => onclickCategory(i)} className={active === i ? styles.categoryActive : styles.categorynoActive}>{item}</div>
          })}
        </div>
        <div className={styles.dropdown}>
          ...
          <ul className={styles.dropdown_content} name="Жанры">
            {genres.map((genre) => {
              return (
                <li>
                  <Link onClick={()=> handleClickGenre(genre.nameGenre)} to={`/genre/${genre._id}`}> {genre.nameGenre}</Link>
                </li>
              );
            })}
          </ul>
        ...
        <button className={styles.dropbtn}>{titleYears}</button>
          <ul className={styles.dropdown_content}>
            {years.map((i)=>{
              return <li><Link onClick={()=> handleClickYears(i)} to={`/years/${i}`}>{i}</Link></li>
            })}
          </ul>
        </div>
        <div className={styles.dropdown}>
        <button className={styles.dropbtn}>{titleCountry}</button>
          <ul className={styles.dropdown_content}>
            {country.map((i)=>{
              return <li><Link onClick={()=> handleClickCountry(i)} to={`/country/${i}`}>{i}</Link></li>
            })}
          </ul>
        </div>
